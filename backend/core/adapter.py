...
# --- CRITICAL FIX: Rename the module import to avoid shadowing ---
try:
    import lxc as native_lxc  # Rename system lib to native_lxc
    USE_NATIVE = True
except ImportError:
    USE_NATIVE = False
...
class PythonLXCAdapter:
    def __init__(self):
        if not USE_NATIVE:
            raise ImportError("Python LXC bindings not available")
        print("DEBUG: Using Native Python LXC Bindings")

    def list_containers(self) -> List[Dict]:
        results = []
        try:
            # FIX: Use native_lxc here, NOT lxc
            names = native_lxc.list_containers()
            for name in names:
                c = native_lxc.Container(name)
    ...
    def backup_container(self, name: str, backup_dir: str) -> str:
        ...
        try:
            if was_running:
                ...
                c.stop()
                # Use "STOPPED" string to be safe
                if not c.wait("STOPPED", 30):
                    raise RuntimeError("Could not stop container (timeout)")
            ...
            print(f"❌ Error during backup of {name}: {e}")
            traceback.print_exc()
            raise e
            
        finally:
            if was_running:
                print(f"DEBUG: Restarting {name}...")
                c.start()


# --- 2. THE SHELL FALLBACK IMPLEMENTATION --- 
class ShellLXCAdapter:
    ...
    def backup_container(self, name, backup_dir):
        if not os.path.exists(backup_dir): os.makedirs(backup_dir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H%M")
        filename = f"{name}_{timestamp}.tar.gz"
        filepath = os.path.join(backup_dir, filename)
        self.stop_container(name)
        subprocess.run(["tar", "-czf", filepath, "-C", "/var/lib/lxc", name], check=True)
        self.start_container(name)
        return filename


# --- INITIALIZATION ---
# This variable 'lxc' is what the rest of the app imports.
# We keep this name so we don't break routers.
try:
    if USE_NATIVE:
